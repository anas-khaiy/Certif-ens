...
const FormationsPage = () => {
    ...
    const handleAddOrUpdate = async (e: React.FormEvent) => {
        ...
        try {
            if (editingFormation) {
                await api.put(`/formations/${editingFormation.id}`, payload);
            } else {
                await api.post('/formations', payload);
            }
            fetchFormations();
            setMessage({ type: 'success', text: `Formation ${editingFormation ? 'mise à jour' : 'ajoutée'} avec succès` });
            closeModal();
        } catch (error: any) {
    ...
    const confirmDelete = async () => {
        if (formationToDelete) {
            try {
                ...
                setIsDeleteModalOpen(false);
                setFormationToDelete(null);
                alert("Impossible de supprimer cette formation car elle est liée à d'autres éléments.");
            }
        }
    };
    ...
    const filteredFormations = formations.filter(f => {
        ...
        return (f.nom || '').toLowerCase().includes(search);
    });
            ...
            <div className="space-y-6 animate-fade-in px-4 md:px-0">
                <div className="flex flex-col gap-6">
                    <div>
                        ...
                        <p className="text-text-muted mb-4">Gérez les types de formations (ex: Formation Initiale, Formation Continue).</p>
                            ...
                            <tbody className="divide-y divide-glass-border">
                                {currentItems.length > 0 ? (
                                    currentItems.map((formation) => (
                                        <tr key={formation.id} className="hover:bg-surface-hover/30 transition-colors">
                                            <td className="font-bold text-text p-4 capitalize">{formation.nom}</td>
                                            <td className="p-4">
                                                <div className="flex justify-center gap-2">
                                                    <button onClick={() => openModal(formation)} className="w-9 h-9 p-0 text-primary hover:bg-primary/10 rounded-xl transition-all icon-container">
                                                        <Edit2 size={16} />
                                                    </button>
                                                    <button onClick={(e) => { e.stopPropagation(); handleDeleteClick(formation.id); }} className="w-9 h-9 p-0 text-error hover:bg-error/10 rounded-xl transition-all icon-container">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan={2} className="py-20 text-text-muted font-medium text-center">Aucune formation trouvée</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                    ...
                    {filteredFormations.length > 0 && (
                        <div className="pagination border-t border-glass-border bg-surface-hover/10 px-6 py-4 flex items-center justify-between">
                            <div className="pagination-info">
                                Affichage <span className="text-text font-bold">{(currentPage - 1) * itemsPerPage + 1}</span> à <span className="text-text font-bold">{Math.min(currentPage * itemsPerPage, filteredFormations.length)}</span> sur <span className="text-text font-bold">{filteredFormations.length}</span> formations
                            </div>
                    ...
                    <div className="modal-overlay" onClick={closeModal}>
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-bold mb-8 flex items-center gap-3">
                                ...
                                {editingFormation ? 'Modifier' : 'Ajouter'} Formation
                            </h3>
                            ...
                            <form onSubmit={handleAddOrUpdate} className="space-y-6">
                                <div className="form-group">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            placeholder="Ex: Formation Initiale"
                                            value={formData.nom}
                                ...
                                <div className="pt-6 flex gap-4">
                                    ...
                                    <button type="submit" disabled={isSubmitting} className="flex-1 primary h-12 disabled:opacity-50 disabled:cursor-not-allowed">
                                        {isSubmitting ? <Loader2 size={18} className="animate-spin mx-auto" /> : (editingFormation ? 'Enregistrer' : 'Créer la formation')}
                                    </button>
                                </div>
                            </form>
                        </motion.div>
