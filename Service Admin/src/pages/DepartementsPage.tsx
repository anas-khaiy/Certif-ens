...
const DepartementsPage = () => {
    ...
    const handleAddOrUpdate = async (e: React.FormEvent) => {
        ...
        try {
            if (editingDepartement) {
                await api.put(`/departements/${editingDepartement.id}`, payload);
            } else {
                await api.post('/departements', payload);
            }
            fetchDepartements();
            setMessage({ type: 'success', text: `Département ${editingDepartement ? 'mis à jour' : 'ajouté'} avec succès` });
            closeModal();
        } catch (error: any) {
    ...
    const confirmDelete = async () => {
        if (departementToDelete) {
            try {
                await api.delete(`/departements/${departementToDelete}`);
                fetchDepartements();
                ...
                setIsDeleteModalOpen(false);
                setDepartementToDelete(null);
                alert("Impossible de supprimer ce département car il est lié à d'autres données.");
            }
        }
    };
    ...
    const currentItems = filteredDepartements.slice(
        ...
        <>
            <div className="space-y-6 animate-fade-in px-4 md:px-0">
                <div className="flex flex-col gap-6">
                    <div>
                        <h2 className="text-3xl font-bold">Gestion des Départements</h2>
                        <p className="text-text-muted mb-4">Configurez les départements de l'établissement.</p>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-surface-hover/50 text-text-muted text-xs uppercase tracking-widest">
                                    <th className="text-left p-4">Nom du Département</th>
                                ...
                                {currentItems.length > 0 ? (
                                    currentItems.map((dept) => (
                                        <tr key={dept.id} className="hover:bg-surface-hover/30 transition-colors">
                                            <td className="font-bold text-text p-4 capitalize">{dept.nom}</td>
                                            <td className="p-4">
                                                <div className="flex justify-center gap-2">
                                                    <button onClick={() => openModal(dept)} className="w-9 h-9 p-0 text-primary hover:bg-primary/10 rounded-xl transition-all icon-container">
                                                        <Edit2 size={16} />
                                                    </button>
                                                    <button onClick={(e) => { e.stopPropagation(); handleDeleteClick(dept.id); }} className="w-9 h-9 p-0 text-error hover:bg-error/10 rounded-xl transition-all icon-container">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan={2} className="py-20 text-text-muted font-medium text-center">Aucun département trouvé</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                    ...
                    {filteredDepartements.length > 0 && (
                        <div className="pagination border-t border-glass-border bg-surface-hover/10 px-6 py-4 flex items-center justify-between">
                            <div className="pagination-info">
                                Affichage <span className="text-text font-bold">{(currentPage - 1) * itemsPerPage + 1}</span> à <span className="text-text font-bold">{Math.min(currentPage * itemsPerPage, filteredDepartements.length)}</span> sur <span className="text-text font-bold">{filteredDepartements.length}</span> départements
                            </div>
                    ...
                    <div className="modal-overlay" onClick={closeModal}>
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-bold mb-8 flex items-center gap-3">
                                ...
                                {editingDepartement ? 'Modifier' : 'Ajouter'} Département
                            </h3>
                            ...
                            <form onSubmit={handleAddOrUpdate} className="space-y-6">
                                <div className="form-group">
                                    ...
                                    <div className="relative">
                                        <div className="search-icon" style={{ left: '16px' }}><Building2 size={18} /></div>
                                        <input
                                            type="text"
                                            required
                                ...
                                <div className="pt-6 flex gap-4">
                                    ...
                                    <button type="submit" disabled={isSubmitting} className="flex-1 primary h-12 disabled:opacity-50 disabled:cursor-not-allowed">
                                        {isSubmitting ? <Loader2 size={18} className="animate-spin mx-auto" /> : (editingDepartement ? 'Enregistrer' : 'Créer le département')}
                                    </button>
                                </div>
                            </form>
                        </motion.div>
                    ...
                    <div className="modal-overlay" onClick={() => setIsDeleteModalOpen(false)}>
                        ...
                        >
                            ...
                            <p className="text-text-muted mb-6">
                                Voulez-vous vraiment supprimer ce département ? Cette action est irréversible.
                            </p>
