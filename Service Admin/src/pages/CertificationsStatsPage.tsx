...
const CertificationsStatsPage = () => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const response = await api.get('/stats/certifications');
                setData(response.data);
            } catch (error) {
                console.error("Failed to fetch certifications stats", error);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const recentCerts = data?.recentCertifications || [];

    // Pagination Logic
    const totalPages = Math.ceil(recentCerts.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const currentItems = recentCerts.slice(startIndex, endIndex);
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-3xl font-bold">Statistiques des Certifications</h2>
                    <p className="text-text-muted">Suivi détaillé des diplômes et taux de réussite.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="glass p-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm text-text-muted">Total Délivré</p>
                            <h3 className="text-2xl font-bold">{data?.totalIssued || 0}</h3>
                        </div>
                    </div>
                </div>
                <div className="glass p-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-bold">{data?.totalPending || 0}</h3>
                        </div>
                    </div>
                </div>
                <div className="glass p-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-bold">{data?.avgScore || 0}%</h3>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={monthlyData}>
                                ...
                                <Tooltip
                                    ...
                                    itemStyle={{ color: '#fff' }}
                                />
                                <Line type="monotone" dataKey="issued" stroke="#6366f1" strokeWidth={3} dot={{ r: 4, fill: '#6366f1' }} name="Délivrées" />
                                <Line type="monotone" dataKey="pending" stroke="#f59e0b" strokeWidth={3} dot={{ r: 4, fill: '#f59e0b' }} name="En attente" />
            ...
            <div className="glass overflow-hidden shadow-xl" style={{ marginTop: '30px' }}>
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-glass-border">
                            {currentItems.length > 0 ? (
                                currentItems.map((cert: any) => (
                                    <tr key={cert.id} className="hover:bg-surface-hover/30 transition-colors">
                                        ...
                                        <td className="p-4 text-center text-text-muted">
                                            {cert.date ? new Date(cert.date).toLocaleDateString('fr-FR') : '-'}
                                        </td>
                                ...
                                <tr>
                                    <td colSpan={5} className="p-10 text-center text-text-muted">Aucune certification récente trouvée.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="pagination border-t border-glass-border bg-surface-hover/10 px-6 py-4 flex items-center justify-between">
                    <div className="pagination-info">
                        Affichage <span className="text-text font-bold">{recentCerts.length > 0 ? startIndex + 1 : 0}</span> à <span className="text-text font-bold">{Math.min(endIndex, recentCerts.length)}</span> sur <span className="text-text font-bold">{recentCerts.length}</span> certifications
                    </div>
...
export default CertificationsStatsPage;
