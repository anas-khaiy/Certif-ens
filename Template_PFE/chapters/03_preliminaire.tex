...
Ce chapitre a pour objectif de cadrer les besoins auxquels le projet \textbf{Certif-fun} doit répondre, qu’ils soient liés aux fonctionnalités principales, aux contraintes techniques ou encore à l’expérience utilisateur. Il permet également de clarifier l’identité des différents acteurs, ainsi que leur rôle dans le système. Enfin, il détaille la logique de fonctionnement au travers de cas d’utilisation et de l’analyse des besoins qui guideront la conception technique.
...
Les besoins fonctionnels représentent les actions spécifiques que le système Certif-fun doit être capable d'exécuter pour satisfaire les utilisateurs finaux.

\subsubsection{Liste des fonctionnalités principales}
L'application propose les services essentiels suivants :
\begin{itemize}
    \item \textbf{Gestion des utilisateurs :} Administration centralisée des comptes (Admin, Formateur, Apprenant) avec contrôle d'accès sécurisé.
    \item \textbf{Vérification d'identité :} Authentification sécurisée avant l'accès à l'examen combinant scan de carte (QR Code) et biométrie faciale.
...
Pour illustrer la valeur ajoutée de Certif-fun, voici trois scénarios métiers types impliquant chaque acteur du système :
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Web Audio API :} Pour l'analyse fréquentielle et la détection intelligente des bruits suspects et de la parole en temps réel.
    \end{itemize}
...
L'identification des acteurs consiste à définir les différents profils qui interagiront avec le système Certif-fun, ainsi que leurs droits et responsabilités respectifs.
...
Les cas d'utilisation (\textit{Use Cases}) permettent de formaliser les interactions entre les acteurs et le système Certif-fun \cite{benabderrezak2025cas}. Ils offrent une vision claire des fonctionnalités du point de vue de l'utilisateur final.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\linewidth]{figures/capture_use_case.png}
    \caption{Diagramme de cas d'utilisation global de Certif-fun.}
...
\begin{figure}[H]
    ...
    \caption{Diagramme de processus métier global (BPMN) de Certif-fun.}
