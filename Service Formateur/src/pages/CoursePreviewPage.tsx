...
const CoursePreviewPage: React.FC = () => {
    ...
    useEffect(() => {
        const initData = async () => {
            ...
            try {
                ...
                // The current service is Service Formateur, so all authenticated users are formateurs
                let isFormateur = true;
                ...
                if (isFormateur) {
                    ...
                    const enrollResponse = await learnerApi.get('/enrollments/my');
                    const enrollments = enrollResponse.data as any[];
                    accepted = enrollments.some(
                        (e: any) => String(e.course?.id) === String(id) && e.status === 'ACCEPTED'
                    );

                    if (!accepted) {
                        // 2.1 Check if course is in an accepted bundle
                        try {
                            const bundleAccessRes = await api.get(`/bundles/trainer/check-course-access/${id}`);
                            if (!bundleAccessRes.data.hasAccess) {
                                setHasAccess(false);
                                setAccessChecked(true);
                                return;
                            }
                            ...
                            console.error("Failed to check bundle course access:", err);
                            setHasAccess(false);
                            setAccessChecked(true);
                            return;
                        }
                ...
                let progress = {} as any;
                let results = [] as any[];
                let tpSubmits = [] as any[];

                if (isFormateur) {
                    // Formateur automatically has all sections "completed" so they can freely navigate
                    const allSubSectionIds = foundCourse.sections?.flatMap((s: any) => 
                        s.subSections?.map((ss: any) => ss.id) || []
                    ) || [];
                    progress = {
                        completedSubSectionIds: allSubSectionIds.join(',')
                    };
                } else {
                    const [progressRes, quizResultsRes, tpSubmissionsRes] = await Promise.all([
                        learnerApi.get(`/progress/${id}`),
                        learnerApi.get(`/progress/${id}/quizzes`),
                        learnerApi.get(`/progress/${id}/tps`)
                    ]);
                    
                    progress = progressRes.data;
                    results = quizResultsRes.data as any[];
                    tpSubmits = tpSubmissionsRes.data as any[];
                }

                setQuizResults(results);
                setTpSubmissions(tpSubmits);
                ...
                if (!isFormateur && foundCourse.deadlineDate) {
                    ...
                    if (isNaN(deadlineTime)) {
                        ...
                        if (!hasExamResult) {
                            learnerApi.post(`/progress/${id}/quiz/final_exam`, {
                                score: 0,
                                passed: false,
                                answers: "[]",
    ...
    useEffect(() => {
        const updateBackendProgress = async () => {
            ...
            try {
                await learnerApi.post(`/progress/${id}`, {
    ...
    const handleTpSubmit = async () => {
        ...
        try {
            const res = await learnerApi.post(`/progress/${id}/tp/${activeSubSectionId}`, { link: tpLink });
    ...
    const handleSubmitQuiz = (quiz: Quiz) => {
        ...
        // Strictly adhere to passing threshold.
        const isPassed = totalQuestions > 0 && roundedUserScore >= passingScorePercent;
        ...
        if (isExamActive) {
            ...
            if (id && activeSubSection?.quiz) {
                ...
                learnerApi.post(`/progress/${id}/quiz/${quizId}`, {
                    score: roundedUserScore,
                    passed: isPassed,
                    answers: answersData
                })
                        ...
                        <div className={`w-full h-full sm:h-auto ${course?.finalExam?.settings?.isAiDetectionEnabled ? 'max-w-5xl' : 'max-w-xl'} bg-surface sm:rounded-[3rem] border-x border-glass-border shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-x-hidden animate-slide-up flex flex-col lg:flex-row min-h-screen sm:min-h-[500px] relative`}>
                            ...
                            <div className={`w-full ${course?.finalExam?.settings?.isAiDetectionEnabled ? 'lg:w-[55%]' : 'w-full'} p-6 sm:p-16 flex flex-col justify-between text-text bg-surface relative`}>
                                <div>
                                    ...
                                    <div className="flex flex-col items-center sm:items-end gap-3 mb-6 sm:mb-8">
                                        ...
                                        <div className="hidden sm:block w-24 h-1 bg-gradient-to-l from-primary to-transparent rounded-full opacity-30 mt-1" />
                                    </div>
