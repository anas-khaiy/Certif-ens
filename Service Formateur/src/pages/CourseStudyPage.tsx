    ...
    useEffect(() => {
        const initData = async () => {
            ...
            try {
                ...
                // 2. Check Enrollment Access (Adapted for Trainer)
                let accepted = true;
                try {
                    ...
                    const hasEnrollment = enrollments.some(
                        (e: any) => String(e.course?.id) === String(id)
                    );
                    if (enrollments.length > 0) {
                        accepted = hasEnrollment;
                    }
                } catch (e) {
                    console.warn("Enrollment check failed, bypassing for trainer");
                }
                ...
                // 3. Load Progress & Quiz Results
                let progress: any = null;
                let results: any[] = [];
                let hasFinishedCertif = false;
                ...
                try {
                    ...
                    hasFinishedCertif = results.some(r => 
                        (String(r.quizId) === 'final_exam' || (finalExamId && String(r.quizId) === String(finalExamId))) && 
                        r.passed
                    );

                    setCompletedSubSections(progress.completedSubSectionIds || []);
                    setTimeSpentSummary(progress.timeSpentSeconds || {});
                    
                    // Set active subsection
                    if (progress.lastSubSectionId) {
                        setActiveSubSectionId(String(progress.lastSubSectionId));
                    } else if (foundCourse.sections?.[0]?.subSections?.[0]) {
                        setActiveSubSectionId(String(foundCourse.sections[0].subSections[0].id));
                    }
                } catch (e) {
                    console.warn("Progress load failed, using defaults");
                    if (foundCourse.sections?.[0]?.subSections?.[0]) {
                        setActiveSubSectionId(String(foundCourse.sections[0].subSections[0].id));
                    }
                }
                ...
                if (progress && progress.timeSpentPerSection) {
                    try {
