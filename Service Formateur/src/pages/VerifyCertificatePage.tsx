...
const VerifyCertificatePage = () => {
    ...
    useEffect(() => {
        const verify = async () => {
            try {
                // Call the Trainer backend (port 8081) for verification
                const response = await axios.get(`${API_FORMATEUR}/verify/${enrollmentId}`);
        ...
        return (
            <div className="min-h-screen bg-background text-text flex flex-col items-center pt-8 md:pt-16 pb-20 px-6 relative overflow-x-hidden transition-colors duration-300">
                ...
                <motion.div
                    ...
                    className="glass max-w-md w-full p-10 text-center border-red-500/20 shadow-xl relative z-10 backdrop-blur-xl"
                >
                    <div className="w-20 h-20 bg-red-500/10 rounded-3xl flex items-center justify-center mx-auto mb-6 border border-red-500/20 rotate-3">
    ...
    return (
        <div className="h-screen overflow-y-auto bg-background text-text flex flex-col items-center pt-8 md:pt-16 pb-20 px-6 relative overflow-x-hidden transition-colors duration-300">
            <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
            ...
            >
                ...
                <div className="glass p-8 md:p-12 relative overflow-hidden group">
                    ...
                    <div className="text-center mb-10 relative">
                        ...
                        <h1
                            className="text-3xl md:text-4xl font-black mb-2 tracking-tight uppercase"
                            style={{ color: theme === 'light' ? '#000000' : '#ffffff' }}
                        >
                            {data.courseName}
                        </h1>
                        <p className="text-text-muted uppercase tracking-[0.3em] text-[10px] font-bold">
                            Certification de Réussite {data.type === 'BUNDLE' ? 'de Parcours' : 'de Module'}
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                        <div className="md:col-span-2 bg-surface/40 p-6 rounded-2xl border border-glass-border hover:border-primary/30 transition-all group/item backdrop-blur-sm">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <p className="text-[10px] uppercase tracking-widest text-text-muted font-bold mb-0.5">
                                        {data.type === 'BUNDLE' ? 'Nom du Parcours' : 'Nom du Module'}
                                    </p>
                        ...
                        <div className="bg-surface/40 p-6 rounded-2xl border border-glass-border hover:border-primary/30 transition-all group/item backdrop-blur-sm">
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <p className="text-[10px] uppercase tracking-widest text-text-muted font-bold mb-0.5">
                                        {data.type === 'BUNDLE' ? 'Moyenne du Parcours' : 'Moyenne du Module'}
                                    </p>
                ...
                <div className="mt-8 text-center text-text-muted text-sm">
                    <p>© 2026 ENS-Learning. Tous droits réservés.</p>
                </div>
            </motion.div>
        </div>
    );
