...
import com.certiflow.formateur.model.CourseProgress;
import com.certiflow.formateur.model.QuizResult;
import com.certiflow.formateur.model.TpSubmission;
import com.certiflow.formateur.service.ProgressService;
...
public class ProgressController {
    ...
    public ResponseEntity<CourseProgress> updateProgress(
        ...
        if (body.get("timeSpentPerSection") != null) {
            ...
            if (obj instanceof String) {
                ...
                try {
                    timeSpentPerSection = objectMapper.writeValueAsString(obj);
                } catch (Exception e) {
        ...
        return ResponseEntity.ok(progressService.updateProgress(courseId, auth.getName(), lastSubSectionId, completedId, timeSpentPerSection));
    }
    ...
    public ResponseEntity<QuizResult> saveQuizResult(
        ...
        if (body.get("answers") != null) {
            try {
                answers = objectMapper.writeValueAsString(body.get("answers"));
            } catch (Exception e) {
        ...
        return ResponseEntity.ok(progressService.saveQuizResult(courseId, auth.getName(), quizId, score, passed, answers, cheatingReason));
    }
    ...
    public ResponseEntity<TpSubmission> saveTpSubmission(
            @PathVariable Long courseId,
    ...
    public ResponseEntity<List<TpSubmission>> getTpSubmissions(
            @PathVariable Long courseId,
            Authentication auth) {
        if (auth == null || !auth.isAuthenticated())
            return ResponseEntity.status(401).build();
