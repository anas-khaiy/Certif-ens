...
const SettingsPage = () => {
    ...
    const handleProfileSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            localStorage.setItem('coordinateurNom', profile.nom);
            localStorage.setItem('coordinateurPrenom', profile.prenom);
            setMessage({ type: 'success', text: 'Profil mis à jour avec succès !' });
            window.dispatchEvent(new Event('profileUpdate'));
        } catch (error: any) {
            const errorMsg = error.response?.data?.message || (typeof error.response?.data === 'string' ? error.response?.data : 'Erreur lors de la mise à jour du profil');
    ...
    const handlePasswordSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMsg = error.response?.data?.message || (typeof error.response?.data === 'string' ? error.response?.data : 'Erreur lors de la modification du mot de passe');
            setMessage({
                type: 'error',
                text: errorMsg
            });
